...
inline RowLayout settingsMenuRow(const ScreenModel& m,int index) {
    ScreenModel laid=m;
    laid.transition=1; laid.scroll=float(2*rowSpacing(m)); laid.selection=m.settings.cursor;
    return layoutRow(laid,index);
}
// Menu text starts where an app list row's circle would, so both line up.
inline int settingsMenuLabelX(const ScreenModel& m,const RowLayout& row) {
    return row.iconX-iconRadius(m);
}
inline void settingsFieldCentre(const ScreenModel& m,int index,int& cx,int& cy) {
    if (m.settings.view==SettingsView::DateTime) {
...
inline int settingsFieldHalfWidth(const ScreenModel& m) {
    return offsetPx(m,m.settings.view==SettingsView::DateTime ? 42 : 78);
}
// A field is one element: the up arrow, the value and the down arrow together,
// so a value change repaints the column as a unit.
inline Rect settingsFieldBox(const ScreenModel& m,int index) {
...
inline int settingsArrowHeight(const ScreenModel& m) { return offsetPx(m,30); }
inline Rect settingsArrowBox(const ScreenModel& m,int index,bool up) {
    const Rect box=settingsFieldBox(m,index); const int h=settingsArrowHeight(m);
    return {box.x,up ? box.y : box.y+box.h-h,box.w,h};
}
inline Rect settingsValueBox(const ScreenModel& m,int index) {
    const Rect box=settingsFieldBox(m,index); const int h=settingsArrowHeight(m);
...
inline Rect settingsSeparatorBox(const ScreenModel& m,int index) {
...
inline Rect settingsButtonBox(const ScreenModel& m,int index) {
    const int count=settingsButtonCount(m.settings.view);
...
inline Rect settingsTitleBox(const ScreenModel& m) {
    const int h=offsetPx(m,48);
    return {m.width/4,offsetPx(m,76)-h/2,m.width/2,h};
}
inline Rect settingsInfoBox(const ScreenModel& m,int line) {
