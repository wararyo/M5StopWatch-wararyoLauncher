...
画面基底クラスは作業6で `apps/AppScreen.h` として抽出しました。外部アプリ詳細が2つ目の実装に
なった時点です（作業5のストップウォッチを待つ必要はありませんでした）。`SettingsScreen` は
全体計画4.1節の契約と同じ形（`enter / exit / handle / model`）のまま、この基底に乗っています。
経緯は[作業6の構成](../task6/multifirm.md)を参照してください。
...
通知（トースト）は最前面の独立したレイヤーで、`planToast` / `paintToast` が時計・一覧・設定の
**後**に描きます。当初は一覧レイヤーの中で描いていたため、設定の「保存」「キャンセル」ボタンが
通知の上に乗って読めなくなりました。`FramePlan` の重なり解決は登録順ではなく矩形の交差で
伝播するので、z順を決めるのは実際の `paint` の呼び出し順だけです。

通知の幅は文字幅から求め、その高さにおける円の弦幅を上限にします。「準備中」向けの固定216pxでは
「日付が正しくありません」が入りきらなかったためです。中心Yは360で、ボタン（374〜426）との
重なりを減らしつつ弦が広い位置に置いています。上限に達した場合は無言で切らず `fitText` で
`...` に省略します。
