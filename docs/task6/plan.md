...
依存しているのは完了条件の2項目だけで、その実体は `bootSlot` の shutdown コールバックに
計測停止を足すことだけである。
...
よって作業6を先に実装し、上の2項目は作業5の完了時に接続して消化する。
そこまで全体計画の作業6の完了条件は閉じない。

当初は「計測中なら『計測を終了して起動』と示す」（8.2節1）も作業5待ちの項目だったが、
2026-09-22に8.2節1そのものを変更し、Readyのスロットは確認画面を挟まず直接起動することにしたため
この項目はなくなった。経緯は下の「確認画面をやめた」を参照。
...
- **計測連携**: 作業6では計測に一切触れない。フック（shutdownコールバック）だけ用意する。

### 確認画面をやめた（2026-09-22、実機確認後）

当初は全体計画8.2節1に従い、Readyのスロットも詳細画面で「起動」「キャンセル」を確認していた。
実機で触った結果、ランチャーの主目的が起動であるのに毎回1手増えるだけだったので、
**8.2節1を変更し、一覧でReadyの行を決定した時点を最終決定とした**。確認画面は挟まない。
計測中の確認（「計測を終了して起動」）も行わないことにしたので、外部アプリ起動は計測を黙って終了させる。

外部アプリ画面は、Ready以外の状態表示・検証中の表示・起動失敗の表示に役割を絞った。
起動できるスロットの省略前の名前とバージョンは、検証中に開いたまま走査が終わった場合に読める。
**走査の完了を起動の契機にはしない**（開いている画面が勝手に起動してはならない）。
...
- `apps/ExternalAppScreen.{h,cpp}`: 位相 `Browsing / BootCommitting / BootFailed`。
  `enter()` がカタログを見て、Readyならその場で `BootCommitting` に入る（確認画面なし）。
  一度きりの判定なので、開いたあとにReadyへ変わっても起動は始まらない。
  ボタンはどの状態でも「戻る」1つだけで、確定中は0個。
  確定後は**ホームを含む全入力を捨てる**（8.2節3）。失敗後は理由を出してホームを再び受け付ける（8.2節4）。
- `ui/ExternalLayout.h` / `ui/ExternalLayer.{h,cpp}`: 設定と同じ流儀。
  題名・3行・ボタン1で容量6。スロット名と版は外部データなので `fitText` を通す。
...
- `tests/multifirm_tests.cpp` を追加（偽 `SlotService` 相手）。一覧の名前差し替えとdim、
  Readyの行の直接起動、Ready以外で画面が開くだけで起動しないこと、確定中の入力抑止、失敗からの復帰、
  **ホーム後に届いた結果が遷移も起動も起こさないこと**、
  **開いたままスロットがReadyになっても起動しないこと**、
...
ストップウォッチ前提の2項目は作業5の後に消化する。未達は原因と対応を明記し、合格扱いにしない。
