...
時計5要素、一覧5行、通知の計11要素に対し、FramePlanは固定32要素です。

1. 全レイヤーの旧矩形・新矩形・内容を計画する。
2. 消去領域だけでなく、新しい描画が重なる要素にも再描画を伝播する。
3. 消去をすべて終えてから、時計→一覧→通知の順に描く。
...
一覧の行の横位置は、アイコンの中心を1本の円弧に乗せて決めます。行の中心Yと画面中心の差 `dy` から
`iconX = iconHomeX + arcRadius − √(arcRadius² − dy²)` を求めるだけです。`iconHomeX`（= `listMargin` +
アイコン半径 = 58px）は中央に来た行の停止位置で、ベゼルとの間隔がちょうど `listMargin` になります。
`arcRadius` は画面半径の1.2倍（281px）で、ベゼルより緩い弧を描いて縁へ近づくほど寄っていきます。
以前は行の矩形の遠い側の辺から弦を求めていたため、ベゼルとの間隔が15〜26pxで揺れた上に、
まだ画面内にいる `|dy|≈190` で横移動が止まっていました。
`arcRadius` と `listMargin` が軌道の2つのつまみで、`iconCentreX()` 以外にXを決める場所はありません。

行の縦方向は遷移の境界（`offset`）と画面でしか切りません。`listMargin` は弧のための半径方向の余裕なので、
ここでも引くと上下に黒帯が残ります。行はベゼルの下へそのまま滑り出て消えます。

行の矩形はアイコンの左端から画面右端までとします。アプリ名の省略は位置に依存しない
`labelWidth()`（行が最も左に来る中央位置での幅 = 338px）だけで決めるので、スクロールしても文字列は
組み直されず、縁へ振られた行は `...` ではなくベゼルで切れます。画面に入りきる名前に `...` は付きません。
...
### 慣性スクロールの試験実装（2026-09-21）

一覧の解放速度から着地行を一度だけ決め、終速ゼロの三次Hermite補間で180msかけて揃える。
時計↔一覧の遷移は従来の補間を使う。自由減速とスナップの二段階には分けない。

- `ScreenManager::settleList()` の先読み時間は90ms、追加距離の上限は1.5行。
  強めの摩擦を想定した初期値で、先読み時間を小さくすると滑る距離が短くなる。
  行への丸めがあるため、実際の追加距離は最大で約2行になる。
- 初速は解放速度を引き継ぐが、逆方向の着地や行き過ぎを防ぐため補間の接線を制限する。
  現在位置から着地行まで単調に移動し、先頭・末尾を越えない。
- 入力速度は時間差を使って平滑化し、最後の縦移動から80ms以上経過していればゼロにする。
- 補間中の再タッチは即停止する。そのタッチを離すと最寄りの行へ揃え、アプリを起動しない。
  続けて縦ドラッグすればその位置から操作を継続する。A/B・ホームは補間を中断する。
- 慣性で先頭に到達しても時計へ戻らない。停止後は次の描画期限を残さない。

PCテストで速度による着地の差、両方向の範囲・単調性、フレーム間隔非依存、停止・再ドラッグ、
タップ誤起動抑止、ボタン割り込み、古い速度の破棄を確認する。
実機では弱い／強いフリック、指を止めてからの解放、再タッチ、両端での操作を確認し、
滑る距離・着地時間・解放直後のつながりを調整する。現時点では実機確認は未実施。
