...
作成日: 2026-09-25。状態: 2026-09-26完了（[検証記録](10-1-validation.md)）。
...
struct BackgroundInfo {
    ...
    const IconBitmap* icon=nullptr; // 静的・不変の8bit濃淡マスク
    std::optional<uint16_t> suggestedColor; // RGB565。黒も有効な指定色
...
2026-09-26の合意として、StopwatchBackgroundInfo等のFeature側の供給元がアイコンと任意の推奨色を提供する。
StopwatchServiceへ資産や配色を追加せず、ホーム側にもアプリIDからアイコンへの対応表を作らない。
既存`ui/graphics/IconBitmap.h`はM5GFXに依存しないデータ記述子なので再利用する。
記述子と画素は静的・不変でアプリ実行中有効とし、毎回のsampleで画像を確保・コピーしない。
別のアイコンへ切り替える場合も静的資産の参照を差し替え、同じ参照先の画素は書き換えない。
既存のランチャー用画像を使う場合は、画像データと取得経路を共通資産へ分離する。供給元からLaunchRegistryを参照しない。

マスクは幅×高さの8bit濃淡（0が背景、255が前景）。供給元が寸法に対応する有効な画素領域を保証する。
null・null画素・非正寸法はアイコンなしへ正規化し、文字盤が汎用表示へ退避する。
推奨色は任意値として未指定と黒を区別する。文字盤は色やアイコンの使用を強制されず、Forestの白描画も可能。
...
- 比較対象はID・ラベル・アイコン参照・推奨色の有無と値・件数・順序。期限が先へ進んだだけでは内容変更イベントを発行しない。
...
- アイコン参照・推奨色の受け渡しと変更検出、未指定と黒の区別、不正アイコンの正規化、未知IDでも提供資産を保持すること。Serviceが描画資産に依存しないこと。
